...
app.post("/admin/addV", (req, res) => {
    ...
    db.query(
        insertVideo,
        [namev, urlv, urllink1, urllink2, urllink3, namelink1, namelink2, namelink3],
        (err, result) => {
            if (err) throw err;
            if (result) {
...
app.post("/register", (req, res) => {
    ...
    const checkEmailExists = "SELECT * FROM users WHERE name = ? AND email = ?";
            ...
            if (password.length < 4) {
                ...
                if (ConfirmPassword !== password) {
                    ...
                    const insertUserToDatabase =
                        "INSERT INTO users(role, name, email, password) VALUES('user', ?, ?, ?)";
                    ...
                    db.query(
                        ...
                        (err, result) => {
                            if (err) throw err;
                            res.redirect("/register?niawebRegisterSuccess=true");
                        }
                    );
                }
            }
...
app.post("/login", (req, res) => {
    ...
    const CheckEmailInDatabase = "SELECT * FROM users WHERE email = ? OR name = ?";
